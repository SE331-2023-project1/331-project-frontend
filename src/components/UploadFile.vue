...
const onChanged = (files: any | any[]) => {
  if (Array.isArray(files)) {
    const convertedFiles = files.map((file) => {
      return { name: file.name };
    });
    emit('update:modelValue', convertMediaToString(convertedFiles));
  } else {
...
<template>
  <Uploader :server="uploadUrl" @change="onChanged" :media="media" accept="*/*"></Uploader>
</template>
