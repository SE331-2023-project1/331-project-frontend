<template>
    ...
    <div>
      <CommentCard v-if="showCommentCard" :key="comment.id" :comment="comment"></CommentCard>
    </div>
  </template>
...
const saveComment = () => {
    ...
    CommentService.saveComment(Number(prop.id), comment.value)
        .then((response) => {
            ...
            store.updateMessage('You have successfully added a new event for ' + response.data);
            setTimeout(() => {
                store.resetMessage();
            }, 3000);
        })
